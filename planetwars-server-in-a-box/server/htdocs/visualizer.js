var Visualizer = {
    ...
    config : {
	  ...
	  planet_id_font: '12px Arial,Helvetica',
    ...
    drawFrame: function(frame) {
        ...
        for(var i = 0; i < this.planets.length; i++) {
			...
			ctx.font = this.config.planet_font;
            ctx.fillStyle = "#fff";
            ctx.fillText(planet.numShips, disp_x, this.canvas.height - disp_y + 8);
            ctx.font = this.config.planet_id_font;
			ctx.fillStyle = "#dddd";
			ctx.fillText(z + ' +' + planet.growthRate, disp_x, this.canvas.height - disp_y - 8);
        }
